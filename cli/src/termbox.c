...
int mapValRGB(int val) {
	float f = (float)val / 0b00011111;
	return f * 255;
}
...
#define TB_DEFAULT_FG_RGB 0b0111111111111111

static void write_sgr(uint16_t fg, uint16_t bg) {
    char buf[32];

    if (outputmode != TB_OUTPUT_RGB && fg == TB_DEFAULT && bg == TB_DEFAULT)
        return;
	...
	case TB_OUTPUT_RGB:
		...
		{
			int r, g, b;
			r = mapValRGB((fg >> (5 * 2)) & 0b00011111);
			g = mapValRGB((fg >> (5 * 1)) & 0b00011111);
			b = mapValRGB((fg >> (5 * 0)) & 0b00011111);
			WRITE_LITERAL("38;2;");
			WRITE_INT(r);
			WRITE_LITERAL(";");
			WRITE_INT(g);
			...
			WRITE_INT(b);
			WRITE_LITERAL(";");
			r = mapValRGB((bg >> (5 * 2)) & 0b00011111);
			g = mapValRGB((bg >> (5 * 1)) & 0b00011111);
			b = mapValRGB((bg >> (5 * 0)) & 0b00011111);
			WRITE_LITERAL("48;2;");
			WRITE_INT(r);
			WRITE_LITERAL(";");
			WRITE_INT(g);
...
#define LAST_ATTR_INIT 0xFFFF
    ...
    if (fg != lastfg || bg != lastbg) {
        ...
        if (outputmode != TB_OUTPUT_RGB && fg & TB_BOLD)
            bytebuffer_puts(&output_buffer, funcs[T_BOLD]);
        if (outputmode != TB_OUTPUT_RGB && bg & TB_BOLD)
            bytebuffer_puts(&output_buffer, funcs[T_BLINK]);
        if (outputmode != TB_OUTPUT_RGB && fg & TB_UNDERLINE)
            bytebuffer_puts(&output_buffer, funcs[T_UNDERLINE]);
        if (outputmode != TB_OUTPUT_RGB && (fg & TB_REVERSE) || (bg & TB_REVERSE))
